setwd('...') #Change this as appropirate
library(mgcv)
library(maps)
library(mapdata)
library(spacetime)
...
#BERING SEA: Import the pollock egg and larval data from Bering Sea. 

pk_egg<-read.csv(file = 'PollockEgg.csv', header=TRUE, check.names=TRUE)
pk_larvae<- read.csv(file = 'PollockLarvae.csv', header=TRUE, check.names=TRUE)

#Select data for constraint analyses, including stations that are <30km away from the closest positive catch
subset.egg<-pk_egg[pk_egg$dist<30000,c('LARVALCATCHPER10M2','YEAR_','LAT','LON','doy')]
subset.larvae<-pk_larvae[pk_larvae$dist<30000,c('LARVALCATCHPER10M2','YEAR_','LAT','LON','doy')]
names(subset.egg)<-c('LARVALCATCHPER10M2','year','lat','lon','doy')
names(subset.larvae)<-c('LARVALCATCHPER10M2','year','lat','lon','doy')

#Juvenile and adults
pk_adults_length<-read.csv(file = 'Pollockcatch.csv', header=TRUE, check.names=TRUE)
pk_adults_catch<-read.csv(file = 'Pollocklength.csv', header=TRUE, check.names=TRUE)
#Only consider overlapping years of eggs, larvae and juv+adults
#Check sample sizes
years_eggs<-sort(unique(subset.egg$year))
years_larvae<-sort(unique(subset.larvae$year))
years_j_a<-sort(unique(pk_adults_length$YEAR))
years_common<-intersect(years_j_a,intersect(years_larvae,years_eggs))
subset.egg<-subset.egg[subset.egg$year%in%years_common,]
subset.larvae<-subset.larvae[subset.larvae$year%in%years_common,]
pk_adults_length<-pk_adults_length[pk_adults_length$YEAR%in%years_common,]
pk_adults_catch<-pk_adults_catch[pk_adults_catch$YEAR%in%years_common,]

nrow(subset.egg);nrow(subset.egg[subset.egg$LARVALCATCHPER10M2>0,]);range(subset.egg$doy)
nrow(subset.larvae);nrow(subset.larvae[subset.larvae$LARVALCATCHPER10M2>0,]);range(subset.larvae$doy)
nrow(pk_adults_catch);nrow(pk_adults_catch[pk_adults_catch$CPUE_NOHA>0,]);range(pk_adults_catch$doy)
...
grid.extent$doy<-median(pk_egg$doy)
...
#Calculate distance of each grid point to closest 'observation': 
#determining what the 'observation' is a critical juncture 
#I am using 'observations' as location sampled and with positive catches in all data set, 
#regardless of stage being considered
for(i in 1:nrow(grid.extent)){
  dist<-distance.function(grid.extent$lat[i],grid.extent$lon[i],
                          c(subset.egg$lat,subset.larvae$lat,pk_adults_length$LAT),
...
#Assign a within sample year and doy to the grid data frame and a vector to fill in cum biomass and habitat
grid.extent$year<-2005
grid.extent$doy<-median(pk_larvae$doy)
grid.extent$cum.b<-NA
grid.extent$cum.h<-NA

gam.pklv<-gam(log(LARVALCATCHPER10M2)~factor(year)+s(lon,lat)+s(doy),data=subset.larvae[subset.larvae$LARVALCATCHPER10M2>0,])
summary(gam.pklv)
...
dev.copy(jpeg,'pk_maps_hires.jpg',height=4,width=10,res=600,units='in')
dev.off()
...
barplot(c(var_ratio.egg,var_ratio.lv,all.var),
        ...
        xlab='Stage or size (mm)',main="MSE ratio",ylim=c(0,1))
box()
...
dev.copy(jpeg,'pk_metrics_hires.jpg',height=7,width=10,res=600,units='in')
dev.off()
